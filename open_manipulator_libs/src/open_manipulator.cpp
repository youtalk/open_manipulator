...
void OpenManipulator::calculationProcess(double tick_time, JointWaypoint* goal_joint_value, JointWaypoint* goal_tool_value)
{
  *goal_joint_value = getJointGoalValueFromTrajectoryTickTime(tick_time);
  *goal_tool_value  = getToolGoalValue();
}

void OpenManipulator::communicationProcessToActuator(JointWaypoint goal_joint_value, JointWaypoint goal_tool_value)
{
  receiveAllJointActuatorValue();
  receiveAllToolActuatorValue();
