...
OpenManipulatorController::OpenManipulatorController(std::string usb_port, std::string baud_rate)
    :node_handle_(""),
     ...
     moving_trajectory_state_(false),
...
void *OpenManipulatorController::commTimerThread(void *param)
{
  ...
  while(controller->comm_timer_thread_state_)
  {
    ...
    if(controller->moving_trajectory_state_ == true)
    {
...
//         log::println(" ");
        ...
        controller->moving_trajectory_state_ = false;
...
void *OpenManipulatorController::calThread(void *param)
{
  ...
  while(controller->cal_thread_state_)
  {
    ...
    if(tempJointWayPoint.size() != 0)
    {
      ...
      {
        if(!controller->open_manipulator_.getMovingState() && trajectory_progress < 100) // error
        {
          char str[100];
          sprintf(str, "[cal thread] Trajectory creation failed : %d %%", trajectory_progress);
          log::error(str);
          controller->cal_thread_state_ = false;
          controller->moving_trajectory_state_ = true;
        }
        else
        {
          log::print("[cal thread] Trajectory creation :",trajectory_progress , 0);
          log::print(" %\r");
        }
      }
      /////////
...
void OpenManipulatorController::trajectoryBufferClear()
{
  moving_trajectory_state_ = false;
                                             ...
                                             open_manipulator_msgs::SetActuatorState::Response &res)
{
  ...
  {
    log::println("[Set actuator state] Wait a second for actuator enable", "GREEN");
    waitCommThreadToTerminate();
  ...
  {
    log::println("[Set actuator state] Wait a second for actuator disable", "GREEN");
    waitCommThreadToTerminate();
    open_manipulator_.disableAllActuator();
    startCommTimerThread();
  }
